...
<cfcomponent output="false" accessors="true">
  <!--- Allows for implicit (get/set)ters --->
  <cfproperty name="ApiKey" type="string" hint="Project API key to use when POSTing data to Airbrake">
  <cfproperty name="Environment" type="string" hint="Name of the environment we're running in">
  <cfproperty name="UseSSL" type="boolean" hint="Should we use SSL Encryption when POSTing to Airbrake?">
  ...
  >
	...
	<cffunction name="getEndpointURL" access="public" returntype="string" output="no"
    hint="Get the endpoint URL to POST to"
  >
		<cfreturn (getUseSSL() ? variables.airbrake_endpoint.secure : variables.airbrake_endpoint.default)>
	</cffunction>


  <cffunction name="send" access="public" returntype="struct" output="No"
    hint="Send an error notification to Airbrake"
  >
  ...
  <cffunction name="exceptionHandler" access="public" returntype="void" output="no"
...
<!--- PRIVATE FUNCTIONS --->
  ...
  >
	...
	<cffunction name="errorToStruct" access="private" returntype="struct" output="no"
